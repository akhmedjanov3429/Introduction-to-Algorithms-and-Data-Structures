 ...
 *************************************************************************/
 ...
 public class TOY { 
     ...
     public static String Decode(String Input_tape) {
        return toHex(Integer.parseInt(Input_tape.replace('-', '0').replace('*', '1'), 2));
    }
 
     public static String Encode(String Input_hex_string) {
         // Getting our Binary String
         String binStr = Integer.toBinaryString(fromHex(Input_hex_string));
         int bLen = binStr.length();
 
         for (int i = 0; i <= (15 - bLen); i++) {
             binStr = "0" + binStr;
         }
         bLen = binStr.length();
         String encStr = "";
 
         // Using for loop to encode
         for (int i = 0; i < bLen; i++) {
             if (binStr.charAt(i) == '0'){encStr = encStr + "-";}
             else{ encStr = encStr + "*";}
         }
         return encStr;
     }
     ...
     public void run() {
         ...
         while (true) {
             ...
             if ((addr == 255 && op == 8) || (reg[t] == 255 && op == 10)) {
                 String Input_1 = StdIn.readString();
                 String Input_2 = StdIn.readString();
                 mem[255] = fromHex(Decode(Input_1+Input_2));}
             ...
             if ((addr == 255 && op == 9) || (reg[t] == 255 && op == 11)){
                 String out = Encode(toHex(mem[255]));
